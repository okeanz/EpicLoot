    ...
    public static class LootRoller
    {
        ...
        public static void Initialize(LootConfig lootConfig)
        {
            ...
            if (Config == null)
            {
                ...
                EpicLootBase.LogErrorForce("Could not load loottables.json! Verify that your json files are installed correctly alongside the EpicLoot.dll file.");
                return;
            }
        ...
        private static void AddItemSets([NotNull] IEnumerable<LootItemSet> itemSets)
        {
            foreach (var itemSet in itemSets)
            {
                ...
                {
                    EpicLootBase.LogError($"Tried to add ItemSet with no name!");
                    continue;
                }

                if (!ItemSets.ContainsKey(itemSet.Name))
                {
                    EpicLootBase.Log($"Added ItemSet: {itemSet.Name}");
        ...
        public static void AddLootTables([NotNull] IEnumerable<LootTable> lootTables)
        {
            ...
            foreach (var lootTable in lootTables.Where(x => x.RefObject == null || x.RefObject == ""))
            {
                ...
                EpicLootBase.Log($"Added loottable for {lootTable.Object}");
            }
            ...
            {
                AddLootTable(lootTable);
                EpicLootBase.Log($"Added loottable for {lootTable.Object} using {lootTable.RefObject} as reference");
            }
        }
        ...
        public static void AddLootTable([NotNull] LootTable lootTable)
        {
            ...
            {
                EpicLootBase.LogError("Loot table missing Object name!");
                return;
            }

            EpicLootBase.Log($"Added LootTable: {key}");
            ...
            else
            {
                ...
                {
                    EpicLootBase.LogError("Loot table missing RefObject name!");
                    return;
                }
                else
                {
        ...
        private static List<GameObject> RollLootTableInternal(LootTable lootTable, int level, string objectName, Vector3 dropPoint, bool initializeObject)
        {
            ...
            if (EpicLootBase.AlwaysDropCheat)
            {
                drops = drops.Where(x => x.Key > 0).ToList();
            }
            else if (Mathf.Abs((int) (EpicLootBase.GlobalDropRateModifier.Value - 1)) > float.Epsilon)
            {
                var clampedDropRate = Mathf.Clamp(EpicLootBase.GlobalDropRateModifier.Value, 0, 4);
            ...
            EpicLootBase.Log($"Available Loot ({loot.Length}) for table: {lootTable.Object} for level {level}");
            foreach (var lootDrop in loot)
            {
                ...
                EpicLootBase.Log($"Item: {itemName} - Rarity Count: {rarity} - Weight: {weight}");
            }
            
            _weightedLootTable.Setup(loot, x => x.Weight);
            var selectedDrops = _weightedLootTable.Roll(dropCount);

            EpicLootBase.Log($"Selected Drops: {lootTable.Object} for level {level}");
            foreach (var lootDrop in selectedDrops)
            {
                ...
                EpicLootBase.Log($"Item: {itemName} - Rarity Count: {rarityLength} - Weight: {lootDrop.Weight}");
            }
            ...
            foreach (var ld in selectedDrops)
            {
                ...
                if (!cheatsActive && EpicLootBase.ItemsToMaterialsDropRatio.Value > 0)
                {
                    var clampedConvertRate = Mathf.Clamp(EpicLootBase.ItemsToMaterialsDropRatio.Value, 0.0f, 1.0f);
                    ...
                    if (replaceWithMats)
                    {
                        ...
                        {
                            EpicLootBase.LogWarning($"Unable to get Prefab for [{lootDrop.Item}]. Continuing.");
                            EpicLootBase.LogWarning($"Error: {e.Message}");
                        }
                        ...
                        if (prefab != null)
                        {
                            ...
                            if (disenchantProducts != null)
                            {
                                foreach (var itemAmountConfig in disenchantProducts)
                                {
                                    ...
                                    {
                                        EpicLootBase.LogWarning($"Unable to get Disenchant Product Prefab for [{itemAmountConfig?.Item ?? "Invalid Item"}]. Continuing.");
                                        EpicLootBase.LogWarning($"Error: {e.Message}");
                                    }
                                    ...
                                    if (materialItemDrop.m_itemData.IsMagicCraftingMaterial())
                                        materialItemDrop.m_itemData.m_variant = EpicLootBase.GetRarityIconIndex(rarity);
                ...
                {
                    EpicLootBase.LogWarning($"Unable to get Gated Item Prefab for [{itemID ?? "Invalid Item"}]. Continuing.");
                    EpicLootBase.LogWarning($"Error: {e.Message}");
                }

                if (itemPrefab == null)
                {
                    EpicLootBase.LogError($"Tried to spawn loot ({itemID}) for ({objectName}), but the item prefab was not found!");
                    continue;
                }
                var item = SpawnLootForDrop(itemPrefab, dropPoint, initializeObject);
                var itemDrop = item.GetComponent<ItemDrop>();
                if (EpicLootBase.CanBeMagicItem(itemDrop.m_itemData) && !ArrayUtils.IsNullOrEmpty(lootDrop.Rarity))
                {
        ...
        private static LootDrop ResolveLootDrop(LootDrop lootDrop)
        {
            ...
            while (needsResolve)
            {
                if (ItemSets.TryGetValue(result.Item, out var itemSet))
                {
                    ...
                    {
                        EpicLootBase.LogError($"Tried to roll using ItemSet ({itemSet.Name}) but its loot list was empty!");
                    }
                ...
                else if (IsLootTableRefence(result.Item, out var lootList))
                {
                    ...
                    {
                        EpicLootBase.LogError($"Tried to roll using loot table reference ({result.Item}) but its loot list was empty!");
                    }
        ...
        private static bool IsLootTableRefence(string lootDropItem, out LootDrop[] lootList)
        {
            ...
            {
                EpicLootBase.LogError($"Tried to get a loot table reference from '{lootDropItem}' but could not parse the level value ({levelText})!");
                return false;
            }
            ...
            if (LootTables.ContainsKey(objectName))
            {
                ...
                EpicLootBase.LogError($"UNLIKELY: LootTables contains entry for {objectName} but no valid loot tables! Weird!");
            }
        ...
        public static MagicItem RollMagicItem(ItemRarity rarity, ItemDrop.ItemData baseItem, float luckFactor)
        {
            ...
            if (rarity == ItemRarity.Legendary)
            {
                ...
                if (legendary == null)
                {
                    ...
                    var rollSetItem = roll < EpicLootBase.SetItemDropChance.Value;
                    EpicLootBase.Log($"Rolling Legendary: set={rollSetItem} ({roll:#.##}/{EpicLootBase.SetItemDropChance.Value})");
                    var availableLegendaries = UniqueLegendaryHelper.GetAvailableLegendaries(baseItem, magicItem, rollSetItem);
                    EpicLootBase.Log($"Available Legendaries: {string.Join(", ", availableLegendaries.Select(x => x.ID))}");
                ...
                if (!UniqueLegendaryHelper.IsGenericLegendary(legendary))
                {
                    ...
                    foreach (var guaranteedMagicEffect in legendary.GuaranteedMagicEffects)
                    {
                        ...
                        if (effectDef == null)
                        {
                            EpicLootBase.LogError($"Could not find magic effect (Type={guaranteedMagicEffect.Type}) while creating legendary item (ID={legendary.ID})");
                            continue;
                        }
            ...
            for (var i = 0; i < effectCount; i++)
            {
                ...
                {
                    EpicLootBase.LogWarning($"Tried to add more effects to magic item ({baseItem.m_shared.m_name}) but there were no more available effects. " +
        ...
        public static MagicItemEffect RollEffect(MagicItemEffectDefinition effectDef, ItemRarity itemRarity, MagicItemEffectDefinition.ValueDef valuesOverride = null)
        {
            ...
            if (valuesDef != null)
            {
                ...
                {
                    EpicLootBase.Log($"RollEffect: {effectDef.Type} {itemRarity} value={value} (min={valuesDef.MinValue} max={valuesDef.MaxValue})");
        ...
        public static List<MagicItemEffect> RollEffects(List<MagicItemEffectDefinition> availableEffects, ItemRarity itemRarity, int count, bool removeOnSelect = true)
        {
            ...
            foreach (var effectDef in effectDefs)
            {
                if (effectDef == null)
                {
                    EpicLootBase.LogError($"EffectDef was null! RollEffects({itemRarity}, {count})");
                    continue;
                }
        ...
        public static List<KeyValuePair<int, float>> GetDropsForLevel([NotNull] LootTable lootTable, int level, bool useNextHighestIfNotPresent = true)
        {
            if (level == 3 && !ArrayUtils.IsNullOrEmpty(lootTable.Drops3))
            {
                ...
                {
                    EpicLootBase.LogWarning($"Duplicated leveled drops for ({lootTable.Object} lvl {level}), using 'Drops{level}'");
                }
            ...
            if (level <= 3 && !ArrayUtils.IsNullOrEmpty(lootTable.Drops))
            {
                if (lootTable.LeveledLoot.Any(x => x.Level == level))
                {
                    EpicLootBase.LogWarning($"Duplicated leveled drops for ({lootTable.Object} lvl {level}), using 'Drops'");
                }
            ...
            EpicLootBase.LogError($"Could not find any leveled drops for ({lootTable.Object} lvl {level}), but a loot table exists for this object!");
            return null;
        }
        ...
        public static LootDrop[] GetLootForLevel([NotNull] LootTable lootTable, int level, bool useNextHighestIfNotPresent = true)
        {
            if (level == 3 && !ArrayUtils.IsNullOrEmpty(lootTable.Loot3))
            {
                if (lootTable.LeveledLoot.Any(x => x.Level == level))
                {
                    EpicLootBase.LogWarning($"Duplicated leveled loot for ({lootTable.Object} lvl {level}), using 'Loot{level}'");
                }
            ...
            if (level <= 3 && !ArrayUtils.IsNullOrEmpty(lootTable.Loot))
            {
                if (lootTable.LeveledLoot.Any(x => x.Level == level))
                {
                    EpicLootBase.LogWarning($"Duplicated leveled loot for ({lootTable.Object} lvl {level}), using 'Loot'");
                }
            ...
            EpicLootBase.LogError($"Could not find any leveled loot for ({lootTable.Object} lvl {level}), but a loot table exists for this object!");
            return null;
        }
        ...
        public static List<MagicItemEffect> RollAugmentEffects(ItemDrop.ItemData item, MagicItem magicItem, int effectIndex)
        {
            ...
            {
                EpicLootBase.LogError($"[RollAugmentEffects] Null inputs: item={item}, magicItem={magicItem}");
                return results;
            }

            if (effectIndex < 0 || effectIndex >= magicItem.Effects.Count)
            {
                EpicLootBase.LogError($"[RollAugmentEffects] Bad effect index ({effectIndex}), effects count: {magicItem.Effects.Count}");
                return results;
            }
            ...
            for (var i = 0; i < augmentChoices && i < availableEffects.Count; i++)
            {
                ...
                if (newEffect == null)
                {
                    EpicLootBase.LogError($"Rolled a null effect: item:{item.m_shared.m_name}, index:{effectIndex}");
                    continue;
                }
        ...
        public static void AddDebugMagicEffects(MagicItem item)
        {
            ...
            {
                EpicLootBase.Log($"AddDebugMagicEffect {ForcedMagicEffect}");
        ...
        {
            var luckFactor = EpicLootBase.GetWorldLuckFactor();

...
namespace EpicLoot.BaseEL.Abilities
{
    public class Ability
    {
        ...
        protected virtual void ActivateStatusEffectAction()
        {
            ...
            {
                EpicLootBase.LogError($"Tried to activate a status effect ability ({AbilityDef.ID}) that was not marked as Action=StatusEffect!");
                return;
            }
            ...
            {
                EpicLootBase.LogError($"Tried to activate a status effect ability ({AbilityDef.ID}) but the status effect name param was missing!");
                return;
            }

            var statusEffect = EpicLootBase.LoadAsset<StatusEffect>(statusEffectName);
            if (statusEffect == null)
            {
                EpicLootBase.LogError($"Tried to activate a status effect ability ({AbilityDef.ID}) but the status effect asset could not be found ({statusEffectName})!");
                return;
            }

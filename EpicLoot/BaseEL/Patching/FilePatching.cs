...
namespace EpicLoot.BaseEL.Patching
{
    [Serializable]
    ...
    public static class FilePatching
    {
        ...
        public static void CheckForOldPatches(DirectoryInfo pluginFolder)
        {
            ...
            if (Directory.Exists(oldPatchFolder))
            {
                ...
                {
                    EpicLootBase.LogWarningForce($"***************************************************");
                    EpicLootBase.LogWarningForce($"Epic Loot Patch Folder Has Moved To:");
                    EpicLootBase.LogWarningForce($"{PatchesDirPath}");
                    EpicLootBase.LogWarningForce($"Please move your patch files. Patch files found in this folder will not be loaded");
                    EpicLootBase.LogWarningForce($"***************************************************");
                }
            }
        }
        ...
        public static void GetAllConfigFileNames(DirectoryInfo pluginFolder)
        {
            ...
            {
                EpicLootBase.LogError($"Error parsing patch directory ({pluginFolder.Name}): {e.Message}");
            }
        ...
        public static void ProcessPatchDirectory(DirectoryInfo dir)
        {
            ...
            {
                EpicLootBase.LogError($"Error parsing patch directory ({dir.Name}): {e.Message}");
            }
        ...
        public static void ProcessPatchFile(FileInfo file)
        {
            ...
            {
                EpicLootBase.LogErrorForce($"Error parsing patch file ({file.Name})! Error: {e.Message}");
                return;
            }

            if (patchFile == null)
            {
                EpicLootBase.LogErrorForce($"Error parsing patch file ({file.Name})! Error: unknown!");
                return;
            }
            ...
            if (!string.IsNullOrEmpty(patchFile.TargetFile) && !string.IsNullOrEmpty(defaultTargetFile) && patchFile.TargetFile != defaultTargetFile)
                EpicLootBase.LogWarningForce($"TargetFile ({patchFile.TargetFile}) specified in patch file ({file.Name}) does not match! If patch file name matches a config file name, TargetFile is unnecessary.");
            ...
            {
                EpicLootBase.LogErrorForce($"TargetFile ({defaultTargetFile}) specified in patch file ({file.Name}) does not exist! {file.Name} will not be processed.");
                return;
            }
            ...
            for (var index = 0; index < patchFile.Patches.Count; index++)
            {
                ...
                EpicLootBase.Log($"Patch: ({file.Name}, {index})\n  > Action: {patch.Action}\n  > Path: {patch.Path}\n  > Value: {patch.Value}");
                ...
                if (string.IsNullOrEmpty(patch.TargetFile))
                {
                    ...
                    {
                        EpicLootBase.LogErrorForce($"Patch ({index}) in file ({file.Name}) requires a specified TargetFile!");
                        continue;
                    }
                ...
                {
                    EpicLootBase.LogErrorForce($"Patch ({index}) in file ({file.Name}) has unknown specified source file ({patch.TargetFile})!");
                    continue;
                }
                ...
                EpicLootBase.Log($"Adding Patch from {patch.SourceFile} to file {patch.TargetFile} with {patch.Path}");
        ...
        public static string ProcessConfigFile(string fileName, string fileText)
        {
            ...
            {
                EpicLootBase.LogErrorForce($"Error parsing config file ({fileName})! Error: {e.Message}");
                return string.Empty;
            }
            ...
            var output = json.ToString(EpicLootBase.OutputPatchedConfigFiles.Value ? Formatting.Indented : Formatting.None);
            return output;
        }
        ...
        public static void ApplyPatch(JObject json, Patch patch)
        {
            ...
            {
                EpicLootBase.LogErrorForce($"Required Patch ({patch.SourceFile}) path ({patch.Path}) failed to select any tokens in target file ({patch.TargetFile})!");
                return;
            }
        ...
        public static void ApplyPatch_Add(JToken token, Patch patch)
        {
            if (patch.Value == null)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Add' but has not supplied a json Value! This patch will be ignored!");
                return;
            }

            if (string.IsNullOrEmpty(patch.PropertyName))
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Add' but has not supplied a PropertyName for the added value! This patch will be ignored!");
                return;
            }
            ...
            if (token.Type == JTokenType.Object)
            {
                ...
                {
                    EpicLootBase.LogWarningForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Add' but a property with the name ({patch.PropertyName}) already exists! The property's value will be overwritten");
            ...
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Add' but has selected a token that is not a json Object! This patch will be ignored!");
            }
        }
        ...
        public static void ApplyPatch_Overwrite(JToken token, Patch patch)
        {
            if (patch.Value == null)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Overwrite' but has not supplied a json Value! This patch will be ignored!");
                return;
            }
            ...
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Overwrite' but did not select a json Object Property or Property Value! This patch will be ignored!");
            }
        }
        ...
        public static void ApplyPatch_Remove(JToken token, Patch patch)
        {
            if (patch.Value != null)
            {
                EpicLootBase.LogWarningForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Remove' but has supplied a json Value. (This patch will still be processed)");
            }
        ...
        public static void ApplyPatch_Append(JToken token, Patch patch, bool appendAll = false)
        {
            ...
            if (patch.Value == null)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action '{actionName}' but has not supplied a json Value! This patch will be ignored!");
                return;
            }
            ...
            if (token.Type == JTokenType.Array)
            {
                if (appendAll)
                {
                    ...
                    {
                        EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'AppendAll' but has provided a value in the source file that is not a json Array!");
                    }
                }
                else
                {
            ...
            else
            {
                ...
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action {actionName} but has selected a token in the target file that is not a json Array!");
            }
        }
        ...
        public static void ApplyPatch_Insert(JToken token, Patch patch, bool after)
        {
            ...
            if (parent == null)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action '{actionName}' but the parent of the selected token is not a container! This patch will be ignored!");
                return;
            }
            ...
            else if (parent.Type == JTokenType.Object)
            {
                if (string.IsNullOrEmpty(patch.PropertyName))
                {
                    EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action '{actionName}' and has selected a property of a json Object, but not provided a PropertyName! This patch will be ignored!");
                    return;
                }
        ...
        public static void ApplyPatch_RemoveAll(JToken token, Patch patch)
        {
            ...
            if (patch.Value != null)
            {
                EpicLootBase.LogWarningForce($"Patch ({patch.SourceFile}, {patch.Path}) has action '{actionName}' but has supplied a json Value! (This patch will still be processed)");
            }
            ...
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action '{actionName}' but selected token is not an Array or Object! This patch will be ignored!");
            }
        }
        ...
        public static void ApplyPatch_Merge(JToken token, Patch patch)
        {
            if (patch.Value == null)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Merge' but has not supplied a json Value! This patch will be ignored!");
                return;
            }

            if (patch.Value.Type != JTokenType.Object)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Merge' but has supplied a json Value that is not an Object! This patch will be ignored!");
                return;
            }

            if (token.Type != JTokenType.Object)
            {
                EpicLootBase.LogErrorForce($"Patch ({patch.SourceFile}, {patch.Path}) has action 'Merge' but has selected a token that is not a json Object! This patch will be ignored!");
                return;
            }

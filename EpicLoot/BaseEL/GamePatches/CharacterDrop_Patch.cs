...
namespace EpicLoot.BaseEL.GamePatches
{
    ...
    public static class Ragdoll_Setup_Patch
    {
        public static void Postfix(Ragdoll __instance, CharacterDrop characterDrop)
        {
            ...
            var characterName = EpicLootBase.GetCharacterCleanName(characterDrop.m_character);
    ...
    public static class Ragdoll_SpawnLoot_Patch
    {
        public static void Postfix(Ragdoll __instance, Vector3 center)
        {
            ...
            {
                EpicLootBase.OnCharacterDeath(characterName, level, center + Vector3.up * 0.75f);
            }
        }
    }
    ...
    public static class CharacterDrop_GenerateDropList_Patch
    {
        public static void Prefix(CharacterDrop __instance)
        {
            if (__instance.m_character != null && __instance.m_character.IsBoss() &&
                EpicLootBase.GetBossTrophyDropMode() != BossDropMode.Default)
            {
                foreach (var drop in __instance.m_drops)
                {
                    if (!(drop.m_prefab == null))
                    {
                        if ((drop.m_prefab.name.Equals("Wishbone") && EpicLootBase.GetBossWishboneDropMode() != BossDropMode.Default) || 
                            (drop.m_prefab.name.Equals("CryptKey") && EpicLootBase.GetBossCryptKeyDropMode() != BossDropMode.Default))
        ...
        {
            if (__instance.m_character != null && __instance.m_character.IsBoss() && EpicLootBase.GetBossTrophyDropMode() != BossDropMode.Default)
            {
                for (var index = 0; index < __result.Count; index++)
                {
                        ...
                        prefab.name.Equals("CryptKey"))
                    {
                        ...
                        switch (EpicLootBase.GetBossTrophyDropMode())
                        {
                            ...
                            case BossDropMode.OnePerPlayerNearBoss:
                                dropCount = Math.Max(Player.GetPlayersInRangeXZ(__instance.m_character.transform.position,  EpicLootBase.GetBossTrophyDropPlayerRange()), playerList.Count(x => Vector3.Distance(x.m_position, __instance.m_character.transform.position) <= EpicLootBase.GetBossTrophyDropPlayerRange()));
                                break;
                        ...
                        EpicLootBase.Log($"Dropping trophies: {dropCount} (mode={EpicLootBase.GetBossTrophyDropMode()})");

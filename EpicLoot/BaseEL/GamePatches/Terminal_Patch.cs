...
namespace EpicLoot.BaseEL.GamePatches
{
    ...
    public static class Terminal_Patch
    {
        ...
        {
            new Terminal.ConsoleCommand("magicitem", "", (args => { MagicItem(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("mi", "", (args => { MagicItem(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("magicitemwitheffect", "",
                (args => { SpawnMagicItemWithEffect(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("mieffect", "",
                (args => { SpawnMagicItemWithEffect(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("magicitemlegendary", "",
                (args => { SpawnLegendaryMagicItem(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("milegend", "", (args => { SpawnLegendaryMagicItem(args.Context, args.Args); }),
                true);
            new Terminal.ConsoleCommand("magicitemset", "", (args => { SpawnMagicItemSet(args.Context, args.Args); }),
                true);
            new Terminal.ConsoleCommand("miset", "", (args => { SpawnMagicItemSet(args.Context, args.Args); }), true);
            new Terminal.ConsoleCommand("checkstackquality", "", (args => { CheckStackQuality(args.Context); }));
            new Terminal.ConsoleCommand("magicmats", "", (args => { SpawnMagicCraftingMaterials(); }), true);
            new Terminal.ConsoleCommand("alwaysdrop", "", (args => { ToggleAlwaysDrop(args.Context); }), true);
            ...
            new Terminal.ConsoleCommand("testtreasuremap", "", (args => { TestTreasureMap(args.Args); }), true);
            new Terminal.ConsoleCommand("testtm", "", (args => { TestTreasureMap(args.Args); }), true);
            ...
            {
                var interval = (args.Length >= 2)
                    ? int.Parse(args[1])
                    : AdventureDataManager.Bounties.GetCurrentInterval();
            ...
            {
                var player = Player.m_localPlayer;
                if (ZoneSystem.instance.FindClosestLocation("Vendor_BlackForest", player.transform.position,
                        out var location))
                {
            ...
            new Terminal.ConsoleCommand("debugluck", "", (args => { LootRoller.DebugLuckFactor(); }));
            new Terminal.ConsoleCommand("refreshmerchant", "", (args =>
            {
                Debug.Log("Refreshing Merchant");
                MerchantPanel.Instance.Panels.ForEach(x=> x.RefreshItems(MerchantPanel.Instance.GetPlayerCurrencies()));
            }));
            new Terminal.ConsoleCommand("debuggamble", "", (args =>
            {
                GambleAdventureFeature.DebugRandom = !GambleAdventureFeature.DebugRandom;
                Debug.Log($"GambleAdventureFeature.DebugRandom = {GambleAdventureFeature.DebugRandom}");
            }));
        }
        ...
        private static IEnumerator TestTreasureMapCoroutine(AdventureSaveData saveData, Heightmap.Biome biome,
            Player player, int count)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                var selectedBiome = biome == Heightmap.Biome.None
                    ? biomes[UnityEngine.Random.Range(0, biomes.Length)]
                    : biome;
                yield return AdventureDataManager.TreasureMaps.SpawnTreasureChest(selectedBiome, player,
                    OnTreasureChestSpawnComplete);
            }
        ...
        private static void OnTreasureChestSpawnComplete(bool success, Vector3 spawnPoint)
        {
            ...
            {
                output =
                    $"> Spawning Treasure Map Chest at <{spawnPoint.x:0.#}, {spawnPoint.z:0.#}> (height:{spawnPoint.y:0.#})";
            }

            Console.instance.AddString(output);
            EpicLootBase.LogWarning(output);
        }

        private static void ToggleAlwaysDrop(Terminal context)
        {
            EpicLootBase.AlwaysDropCheat = !EpicLootBase.AlwaysDropCheat;
            context.AddString($"> Always Drop: {EpicLootBase.AlwaysDropCheat}");
        }

        private static void SpawnMagicCraftingMaterials()
        {
            foreach (var itemPrefab in EpicLootBase.RegisteredItemPrefabs)
            {
                var itemDrop =
        ...
        public static void MagicItem(Terminal context, string[] args)
        {
            ...
            var allItemNames = ObjectDB.instance.m_items
                .Where(x => EpicLootBase.CanBeMagicItem(x.GetComponent<ItemDrop>().m_itemData))
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var dropPoint = Player.m_localPlayer.transform.position + Player.m_localPlayer.transform.forward * 3 +
                                Vector3.up * 1.5f + randomOffset;
        ...
        public static void SpawnMagicItemWithEffect(Terminal context, string[] args)
        {
            if (args.Length < 3)
            {
                EpicLootBase.LogError("Specify effect and item name");
                return;
            }
            ...
            if (!EpicLootBase.CanBeMagicItem(fromItemData))
            {
            ...
            var itemRarity = effectRequirements.AllowedRarities.Count == 0
                ? ItemRarity.Magic
        ...
        private static void SpawnLegendaryItemHelper(string legendaryID, string itemType, Terminal context)
        {
            ...
            if (string.IsNullOrEmpty(itemType))
            {
                EpicLootBase.LogWarning(
                    $"No Item Type Found for LegendaryID {legendaryID} - This would've made a Club.");
                return;
            }
        ...
        private static void ReplaceMagicEffect(ItemDrop.ItemData itemData, MagicItem magicItem, MagicItemEffect effect,
            int index)
        {

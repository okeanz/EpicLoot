...
namespace EpicLoot.BaseEL
{
    ...
    public static class VisEquipment_Patch
    {
        ...
        private static void SetItemFx(GameObject __result, ItemDrop.ItemData equippedItem, Player player)
        {
            ...
            {
                var asset = EpicLootBase.LoadAsset<GameObject>(equipFx);
                if (asset != null)
                {
                    var attachObject = mode == FxAttachMode.Player ? player.transform : __result?.transform;
                    if (attachObject == null)
                    {
                        EpicLootBase.LogError($"Tried to attach FX to item that did not exist. item={equippedItem.m_shared.m_name}, equipFx={equipFx}, mode={mode}");
                        return;
                    }
        ...
        private static void SetTextureOverrides(VisEquipment __instance, List<GameObject> __result, string itemID, ItemDrop.ItemData equippedItem)
        {
            ...
            {
                var chestTexAsset = EpicLootBase.LoadAsset<Texture>(chestTex);
                ...
                {
                    EpicLootBase.LogError($"Missing Texture Override Asset: ChestTex={chestTex}");
                }
            }

            if (!string.IsNullOrEmpty(legsTex))
            {
                var legsTexAsset = EpicLootBase.LoadAsset<Texture>(legsTex);
                ...
                {
                    EpicLootBase.LogError($"Missing Texture Override Asset: LegsTex={legsTex}");
                }
            }
        }
        ...
        {
            var mainTextureAsset = EpicLootBase.LoadAsset<Texture>(mainTexture);
            ...
            {
                EpicLootBase.LogError($"Missing Texture Override Asset: MainTexture={mainTexture}");
            }
        }
        ...
        public static void Humanoid_UnequipItem_Prefix(Humanoid __instance, ItemDrop.ItemData item, bool triggerEquipEffects)
        {
            ...
            if (!string.IsNullOrEmpty(equipFx) && mode == FxAttachMode.Player)
            {
                ...
                if (effect == null)
                {
                    EpicLootBase.LogError($"Unequipped item ({item.m_shared.m_name}) from player that had fx, but could not find fx ({equipFx})!");
                    return;
                }

...
namespace EpicLoot.BaseEL.Adventure.Feature
{
    public class AvailableBountiesListPanel : MerchantListPanel<BountyListElement>
    {
        ...
        public override void RefreshButton(Currencies playerCurrencies)
        {
            ...
            bool allowedToBuy = !(EpicLootBase.EnableLimitedBountiesInProgress.Value &&
                                  bountyInProgressCount >= EpicLootBase.MaxInProgressBounties.Value);

            if (MerchantPanel.AcceptBountyText != null)
            {
                MerchantPanel.AcceptBountyText.text = Localization.instance.Localize(!allowedToBuy ? string.Format("$mod_epicloot_merchant_max_bounties ({0})", EpicLootBase.MaxInProgressBounties.Value): "$mod_epicloot_merchant_acceptbounty");
            }

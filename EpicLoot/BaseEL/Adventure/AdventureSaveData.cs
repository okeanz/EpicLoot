...
namespace EpicLoot.BaseEL.Adventure
{
    ...
    public class BountyInfo
    {
        ...
        public static BountyInfo FromBountyID(string ID)
        {
            ...
            {
                EpicLootBase.LogError($"Bounty {ID} not found");
                return null;
            }
        }
    }
    ...
    public class AdventureSaveData
    {
        ...
        public int[] PurchasedGambles = new int[4];

        [NonSerialized] public bool DebugMode;
        [NonSerialized] public int IntervalOverride;

        public void PurchasedGambleItem(int rarity)
        {
            if(rarity > 3) return;
            PurchasedGambles[rarity]++;
        }

        public void RefreshPurchasedGambles()
        {
            PurchasedGambles = new int[4];
        }
        ...
        public bool PurchasedTreasureMap(int interval, Heightmap.Biome biome, Vector3 position, Vector3 circleOffset)
        {
            if (!DebugMode)
            {
                ...
                {
                    EpicLootBase.LogError($"Player has already purchased treasure map! (interval={interval} biome={biome})");
                    return false;
                }
            }
        ...
        public bool AcceptedBounty(BountyInfo bounty, Vector3 spawnPoint, Vector3 offset)
        {
            ...
            {
                EpicLootBase.LogError($"Player has already accepted bounty! (interval={bounty.Interval} bountyID={bounty.ID})");
                return false;
            }

            if (bounty.State != BountyState.Available)
            {
                EpicLootBase.LogError($"Can only accept available bounties! (interval={bounty.Interval} bountyID={bounty.ID})");
                return false;
            }
